...
- **框架**：SvelteKit 4
- **語言**：TypeScript
- **部署**：Cloudflare Pages (`cloudflare-9qe.pages.dev`)
...
| 組件 | 職責 | 檔案位置 | 技術 |
|------|------|---------|------|
| **Main Worker** | 接收請求、KV 緩存、圖片 URL 重寫 | [hooks.server.ts](../cloudflare-wordpress/src/hooks.server.ts) | SvelteKit |
| **Sync Worker** | WordPress → D1 同步、圖片上傳 R2 | [api/sync/+server.ts](../cloudflare-wordpress/src/routes/api/sync/+server.ts) | SvelteKit API |
| **WordPress Plugin (Sync)** | 監聽 WordPress 事件並觸發同步 | [wp-d1-sync.php](../Wordpress%20Plugin/wp-d1-sync.php) | PHP |
| **WordPress Plugin (Cache)** | 清除 KV 緩存 | [wp-cache-purge.php](../Wordpress%20Plugin/wp-cache-purge.php) | PHP |
| **KV Namespace** | HTML 頁面緩存 | `HTML_CACHE` | Cloudflare KV |
| **D1 Database** | WordPress 數據副本 | `wordpress-cloudflare` | Cloudflare D1 |
| **R2 Bucket** | 媒體文件存儲 | `media-bucket` | Cloudflare R2 |
